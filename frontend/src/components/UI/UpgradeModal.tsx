...
export const UpgradeModal: React.FC<UpgradeModalProps> = ({ isOpen, onClose, onUpgrade }) => {
  ...
  return (
    ...
    >
      <div 
        className="bg-black border-2 border-red-900 max-w-2xl w-full max-h-[90vh] flex flex-col relative overflow-hidden"
      ...
      >
        ...
        {/* Footer Fixo */}
        <div className="p-4 border-t border-red-900/30 bg-black z-20 space-y-3 shrink-0">
          {/* Preço */}
          <div className="border border-red-900/30 bg-red-950/10 p-3">
            <div className="flex items-baseline justify-center gap-2 mb-1">
              <span className="text-3xl font-bold text-white font-mono">R$ 29,90</span>
              <span className="text-sm text-red-400/70">/mês</span>
            </div>
            <p className="text-center text-[10px] text-zinc-500 font-mono">
              Investimento mínimo. Cancele se for fraco.
            </p>
          </div>
          ...
          <div className="space-y-2">
            <button
              onClick={onUpgrade}
              className="w-full py-3 bg-red-900 hover:bg-red-700 text-white transition-all uppercase font-mono text-xs tracking-widest font-bold relative overflow-hidden group"
            >
            ...
            <button
              onClick={onClose}
              className="w-full py-2 border border-zinc-800 text-zinc-500 hover:text-zinc-300 hover:border-zinc-700 transition-colors uppercase font-mono text-[10px] tracking-widest"
            >
          ...
          <div className="text-center pt-2 border-t border-zinc-900">
            <p className="text-[9px] text-zinc-700 uppercase font-mono tracking-widest">
              Processamento seguro via Stripe • O sistema não aceita desculpas
            </p>
          </div>
        </div>
      </div>
